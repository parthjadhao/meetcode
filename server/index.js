...
function adminAutheticate(req,res,next){
    ...
    for (let index = 0; index < admin.length; index++) {
      if (admin[index].username == username && admin[index].id == id) {
        next()
      } 
    }
    // res.status(400).send("please loggin or signup admin")
}

function userAutheticate(req,res,next) {
    const {username,id} = req.headers
    for (let index = 0; index < user.length; index++) {
        if (user[index].username == username && admin[index.id]== id) {
            next()
        }   
    }
    // res.status(400).send("please loggin or signup user")
}
...
app.post('/registerUser', (req,res)=>{
    ...
    userIndex++
    res.json({message:"user account created succesfully",id:registerAdminDetail.id,username:registerAdminDetail.username})
});
...
app.post('/registerAdmin',(req,res)=>{
    ...
    res.json({message:"admin account created succesfully",id:registerAdminDetail.id,username:registerAdminDetail.username})
});
